...
PRO_PATH=$WsRootDir/S103X_Master
CPUCORE=8
D_TIME=`date +%Y%m%d`
...
function main()
{
    if [ ! -d $PRO_PATH/.repo ];then
        ...
        cd $PRO_PATH/NOHLOS/out
            mv S103X_32_Master_QFIL.zip S103X_Master_$D_TIME.zip
            upload_version
        cd -
    else
        echo "存在"
...
function original_build()
{
    ...
    #"修改dailybuild版本号"
    DAILYBUILD_NUMBER=S103X_Master_$D_TIME
...
function pull_code()
{
    ...
    if [ ! -f ./.repo/manifest.xml ] ;then
        ...
        echo "check repo path ssh://$USER@10.80.30.10:29418/LNX_LA_SDM450_S102X_PSW  is right?"
        exit 0
    else
...
function upload_version(){
    smbclient //10.80.10.2/人工智能bg软件部/ -U gaoyuxia%gyx@2019 -c "cd 2_临时软件版本\S103X\S103X_Master_dailybuild;lcd $PRO_PATH/NOHLOS/out;put S103X_Master_$D_TIME.zip"
}
################################
main

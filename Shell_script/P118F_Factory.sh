...
function main()
{
    if [ ! -d $PRO_PATH/.repo ];then
        ...
        cd $PRO_PATH/NOHLOS/out
            mv FlashPackage_SDM450_Factory_QFIL.zip P118F_Factory_$D_TIME.zip
            upload_version            
        cd -
    else
        echo "存在"
...
function original_build()
{
    ...
    #"修改dailybuild版本号"
    DAILYBUILD_NUMBER=P118F_Factory_$D_TIME
...
function pull_code()
{
    ...
    repo init -u ssh://10.80.30.10:29418/LNX_LA_SDM450_PSW/Manifest -m manifest.xml -b Stable_P118F_Factory_BRH --repo-url=ssh://10.80.30.10:29418/Tools/Repo --no-repo-verify --reference="/home1/jinlujiao/mirror/P118F_MIRROR" 

...
def download_code():
    ...
    #开始下载代码
    commands.getoutput("repo init -u %s  -m manifest.xml -b %s --reference=%s --repo-url=ssh://10.80.30.10:29418/Tools/Repo --no-repo-verify" %(code_url,BRANCH,code_mirror) )
    os.chdir(pathroot+"/.repo/manifests")
    tree=ET.parse("manifest.xml")
    root=tree.getroot()
    for child in root.findall("remote"):
        #修改XML中fetch的值
        child.attrib['fetch']="ssh://10.80.30.10:29418/"
